...
1. **Presentation Layer (Apps)**
   ...
   - **API (`/apps/api`):** High-throughput **gRPC API** built with **Python** using Protocol Buffers, serving the UI (via gRPC-Web) and internal systems.
...
4. **Data & Storage Layer**
   - **AWS S3:** Object storage for PDFs, Docling-parsed JSON, and intermediate artifacts.
...
graph TD;
    ...
    S3_Raw_PDFs-->Docling_Parser[Docling Preprocessor];
    Docling_Parser-->S3_Parsed_Data;
...
- **Ingestion:** Use AWS Step Functions to orchestrate the ingestion pipeline (Docling Preprocessing -> Chunking -> Embedding) to handle parallel processing of hundreds of PDFs.
